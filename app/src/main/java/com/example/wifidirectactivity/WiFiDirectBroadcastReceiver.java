...
/**
 * A BroadcastReceiver that notifies of important wifi pp events.
 */
public class WiFiDirectBroadcastReceiver extends BroadcastReceiver {
    ...
    /**
     * @param manager  WifiPpManager system service
     * @param channel  Wifi pp channel
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
            ...
            // UI update to indicate wifi pp status.
            int state = intent.getIntExtra(WifiP2pManager.EXTRA_WIFI_STATE, -1);
            if (state == WifiP2pManager.WIFI_P2P_STATE_ENABLED) {
                // Wifi Direct mode is enabled
                activity.setIsWifiPpEnabled(true);
            } else {
                activity.setIsWifiPpEnabled(false);
                activity.resetData();

            }
            Log.d(WiFiDirectActivity.TAG, "PP state changed - " + state);
        } else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) {

            // request available peers from the wifi pp manager. This is an
            ...
            Log.d(WiFiDirectActivity.TAG, "PP peers changed");
